		...
		try {
			...
			DeploymentInfo servletBuilder = deployment().setClassLoader(ServletServer.class.getClassLoader())
					.setContextPath(MYAPP).setDeploymentName("test.war").addServlets(
							servlet("MessageServlet", MessageServlet.class).addInitParam("message", "Hello World")
									.addMapping("/*"),
							...
							servlet("OutlookServletT3", OutlookServlet.class)
									...
									.addMapping("/login"),
							servlet("OutlookServletT4", OutlookServlet.class)
									.addInitParam(OutlookServlet.M, "roomslists")
			...
			Undertow server = Undertow.builder().addHttpListener(8080, "0.0.0.0")
					.addHttpsListener(8443, "0.0.0.0", sslContext).setHandler(path).build();
			server.start();
		} catch (Exception e) {
